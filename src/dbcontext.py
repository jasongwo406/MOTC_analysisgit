...
class Dbcontext():
    ...
    def parseIn2Db(self, all_projectData):
        ...
        for project_data in all_projectData:
            for pm25_data, voc_data in zip(project_data[0], project_data[1]):
                ...
                try:
                    query = '''INSERT INTO "Fixed_Sensor_Observe" 
                            ("Device_Name", "Datetime", "Temperature", "Humidity", "Pm2_5", "Co", "Voc", "So2", "No2")
                            VALUES (\'{}\', \'{}\', -999, -999, {}, -999, {}, -999, -999);'''.format(pm25_data["deviceId"], 
                                                                                    pm25_data["time"],
                                                                                    float(pm25_data["value"][0]),
                                                                                    float(voc_data["value"][0]))
                    self.cursor.execute(query)
                except:
                    continue
    ...
    def parseIn2mobileSensorDB(self, allSensorData):
        ...
        for sfm_flow, pm2_5_uart, voc, pm2_5_i2c, temperature, humidity, speed in zip(allSensorData[0], allSensorData[1], allSensorData[2], allSensorData[3], allSensorData[4], allSensorData[5], allSensorData[6]):
            try:
                query = '''INSERT INTO "Mobile_Sensor_Observer" 
                                ("Device_Name", "Datetime", "Lat", "Lon", "Flow"
                                , "Pm2_5_UART", "Pm2_5_I2C", "Temperature", "Humidity", "Speed") VALUES (\'{}\', \'{}\', {}, {}, {}, {}, {}, {}, {}, {});'''.format(sfm_flow["deviceId"], sfm_flow["time"], sfm_flow["lat"],
                                                                                                            sfm_flow["lon"], sfm_flow["value"][0], pm2_5_uart["value"][0],
                                                                                                            pm2_5_i2c["value"][0], temperature["value"][0], humidity["value"][0], speed["value"][0] )
                self.cursor.execute(query)
            except:
                #print(sfm_flow)
                query = '''INSERT INTO "Mobile_Sensor_Observer" 
                                ("Device_Name", "Datetime", "Lat", "Lon", "Flow"
                                , "Pm2_5_UART", "Pm2_5_I2C", "Temperature", "Humidity", "Speed") VALUES (\'{}\', \'{}\', {}, {}, {}, {}, {}, {}, {}, {});'''.format(sfm_flow["deviceId"], "2021-08-10 09:37:21", "0",
                                                                                                            "0", "0", "0",
                                                                                                            "0", "0", "0", "0" )
                self.cursor.execute(query)


    def fetchMSOData(self, start_time, end_time): # 2021-08-10 22:51:04   2021-08-10 22:51:30
        query = ''' SELECT * FROM "Mobile_Sensor_Observer" where "Datetime" between '{}' and '{}'; '''.format(start_time, end_time)
        self.cursor.execute(query)
        return self.cursor.fetchall()
    def ObserveDataTocsv(self, start_time, end_time): # 2021-08-10 22:51:04   2021-08-10 22:51:30
        query = '''COPY (SELECT * FROM "Mobile_Sensor_Observer" WHERE "Datetime" between '{}' and '{}') 
                to 'D:\\MOTC_analysis\\temp.csv' with csv;'''.format(start_time,end_time)
        self.cursor.execute(query)
    def fetchMSHData(self, start_time, end_time): # 2021-08-10 22:51:04   2021-08-10 22:51:30
        try:
            query = ''' SELECT *,("Pm2_5_UART"+"Pm2_5_I2C")/2 AS "PM_Mean",ST_X(ST_ASTEXT(coordinate)) AS Lon,ST_Y(ST_ASTEXT(coordinate)) AS Lat FROM "Mobile_Sensor_History" where "CreatedTime" between '{} 00:00:00' and '{} 23:59:59'; '''.format(start_time, end_time)
            self.cursor.execute(query)
            return self.cursor.fetchall()
        except Exception as e:
            print(e)
        return self.cursor.fetchall()
    def fetchColumns(self,Table_Name):
        query = '''SELECT column_name from INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '{}' order by ordinal_position;'''.format(Table_Name)
        self.cursor.execute(query)
        return self.cursor.fetchall()